...
| `grokden-sidebar.css` | **DELETED** (Phase 4) | Global nav rail styles; folded into `ActivityRail.svelte` scoped `<style>`. |
...
| `grokden-final-ui-reset.css` | **DELETED** (Phase 4) | Masks broken terminal/sidebar states via `!important` class overrides. Svelte state owns visibility. |
| `grokden-premium-theme-fix.css` | **DELETED** (Phase 4) | Glass + theme-picker rules folded into `grokden-premium-theme.css` (nav uses `nav.sidebar`). |
| `grokden-interaction-v2.js` | **DELETED** (Phase 4) | Not linked; legacy class toggles on `.ide`. |
| `grokden-ui-hotfix.js` | **SLIMMED** (Phase 4) | Terminal hide-class sync removed; menu hover behavior kept. |
